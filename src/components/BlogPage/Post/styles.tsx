import tw from "twin.macro"

export const BlogPostWrapper = tw.div`w-full flex flex-col justify-center items-center`
export const TitleWrapper = tw.div`w-full flex flex-row justify-center items-center`
export const Title = tw.p`text-offwhite text-5xl font-playfair-display font-bold`
export const SubtitleWrapper = tw.div`w-full mt-4 flex flex-row justify-center items-center`
export const Subtitle = tw.p`text-offpink text-2xl font-playfair-display font-bold`
